...
int get_minimal_weight(double* distances, int* visited, int V) {
    if (!distances || !visited) {
        return -1;
    }
    int i = 0;
    double minimum = MAX_WEIGHT;
    int min_index = -1;
    for (i = 0; i < V; i++) {
...
int Dijkstra_s_algorithm(Heap* heap, double** adjacency_matrix, int N, double* distances, int* visited, int* previous) {
...
double** set_adjacency_matrix(int N) {
    int i;
    int j;
    double** adjacency_matrix = calloc(N, sizeof(double*));
    for (i = 0; i < N; i++) {
        adjacency_matrix[i] = calloc(N, sizeof(double));
    }
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
...
int free_adjacency_matrix(double** adjacency_matrix, int N) {
    if (!adjacency_matrix) {
        return -1;
    }
    int i;
...
int main(void) {
    ...
    double** adjacency_matrix = set_adjacency_matrix(N);
